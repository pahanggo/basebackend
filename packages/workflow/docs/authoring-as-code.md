...
- `type` is `state`, `fork`, or `join`. A `state` node can also carry:
  - `field_policy`: ordered list of `{field, visible, label, type, readonly, custom_field_definition}` — overrides the target model's Backpack form while a record sits here. `readonly: false` additionally makes the field a real editable input on the show-workflow page, saved from there (`POST workflows/show/update`) independently of the model's own Update operation — so a node can grant "edit just these fields, right here" without `operation_settings.update`/`row_actions.update` being enabled at all; each save is recorded on the timeline as an edit step. A dotted (related-model) `field` is never inline-editable, only displayed. `custom_field_definition` is a literal PHP-array-as-string (parsed by `Workflow\Support\CustomFieldDefinitionParser`, never `eval`'d), e.g. `"[\n\"prefix\" => \"$\",\n]"` for a money field's prefix.
