...
<script type="text/javascript">
    jQuery(document).ready(function($) {
        ...
        window.crud.table.exportAllData = async function() {
            $('#loading-overlay-dttbl').css('display', 'flex');

            const headerData = crud.table
                .columns()
                .header();

            const inExport = crud.table.columns()[0].map(function(idx) {
                var $column = crud.table.column(idx);
                var node = headerData[idx];
                return ($column.visible() && $(node).attr('data-visible-in-export') !=
                    'false') || $(node).attr('data-force-export') == 'true';
            });
            ...
            $($('.buttons-collection')[0]).html('Exporting... <span id="export-output"></span>');
            ...
            while (params.start < filtered) {
                ...
                filtered = postResult.recordsFiltered;
                const page = Math.round(params.start / params.length);
                const max = Math.ceil(filtered / params.length);
                results = results.concat(postResult.data.map((cols) => {
                    return cols.map((col) => {
                        var div = document.createElement("div");
                        div.innerHTML = col;
                        let text = div.textContent || div.innerText || "";
                        text = text.trim();
                        if (text.indexOf(
                                'markCheckboxAsCheckedIfPreviouslySelected') > -1) {
                            text = ''
                        }
                        try {
                            var temp = JSON.parse(text);
                            if (temp && temp.length && temp.concat) {
                                text = temp.join(',')
                            }
                        } catch (e) {}
                        return text;
                ...
                $('#export-output').text(`${page} / ${max}`)
            }

            setTimeout(function() {
                $($('.buttons-collection')[0]).html(
                    `<span><i class="la la-download"></i> Export</span>`);
            }, 500);
            ...
            $('#loading-overlay-dttbl').css('display', 'none');
            return response;
        }
