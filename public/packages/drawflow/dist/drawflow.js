/*!
 * Drawflow — vendored from https://github.com/jerosoler/Drawflow
 * (src/drawflow.js, MIT License), unminified (this repo ships no JS build
 * step for vendored assets — see packages/backpack/*, select2, and this
 * package's own docs/architecture history) and locally patched to give
 * connections a real identity. Every change is marked "// PATCHED" inline;
 * search for that to find them all when re-vendoring a newer upstream
 * version. Patched methods: addConnection(), removeSingleConnection(), and
 * the 'connectionSelected' dispatch — each gained an optional connection id
 * so several distinct connections can exist between the exact same
 * (output node, output port, input node, input port), which the unpatched
 * library has no way to represent at all (identity is otherwise purely
 * structural — see packages/workflow's designer canvas.blade.php/
 * connections.blade.php for the consumer side of this).
 */
(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() :
  typeof define === 'function' && define.amd ? define(factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, (global.Drawflow = factory()));
...
class Drawflow {
    ...
    switch (this.ele_selected.classList[0]) {
      ...
      case 'main-path':
        ...
        if(listclassConnection.length > 1){
          this.dispatch('connectionSelected', { output_id: listclassConnection[2].slice(14), input_id: listclassConnection[1].slice(13), output_class: listclassConnection[3], input_class: listclassConnection[4], id: this.connection_selected.parentElement.dataset.edgeId }); // PATCHED: real connection id (see addConnection)
          if(this.reroute_fix_curvature) {
            this.connection_selected.parentElement.querySelectorAll(".main-path").forEach((item, i) => {
  ...
  // PATCHED: optional 5th `connection_id` — when supplied, this connection is
  // always added (the exist-check below is skipped entirely) and the id is
  // stored on both connection entries and stamped onto the SVG as
  // data-edge-id, so several distinct connections can share the very same
  // (output node, output port, input node, input port) tuple — Drawflow's
  // own identity model otherwise has no way to tell them apart at all.
  // Omit connection_id to get the original single-connection-per-port
  // behavior unchanged (e.g. the library's own interactive drag-to-create flow).
  addConnection(id_output, id_input, output_class, input_class, connection_id = null) {
    ...
    if(nodeOneModule === nodeTwoModule) {
      ...
      if(exist === false) {
        //Create Connection
        this.drawflow.drawflow[nodeOneModule].data[id_output].outputs[output_class].connections.push( {"node": id_input.toString(), "output": input_class, "id": connection_id});
        this.drawflow.drawflow[nodeOneModule].data[id_input].inputs[input_class].connections.push( {"node": id_output.toString(), "input": output_class, "id": connection_id});
        ...
        //Draw connection
          ...
          if (connection_id !== null) { connection.dataset.edgeId = connection_id; } // PATCHED
          connection.appendChild(path);
          this.precanvas.appendChild(connection);
          this.updateConnectionNodes('node-'+id_output);
          this.updateConnectionNodes('node-'+id_input);
        ...
        this.dispatch('connectionCreated', { output_id: id_output, input_id: id_input, output_class:  output_class, input_class: input_class, id: connection_id});
      }
    }
  }
  ...
  // PATCHED: optional 5th `connection_id` — when supplied, matches/removes
  // by that id instead of by (node, port) alone, and removes the specific
  // DOM element carrying that data-edge-id (querySelector on the old
  // class-only selector would only ever find the FIRST of several
  // same-tuple connections, removing the wrong one). See addConnection().
  removeSingleConnection(id_output, id_input, output_class, input_class, connection_id = null) {
    var nodeOneModule = this.getModuleFromNodeId(id_output);
    var nodeTwoModule = this.getModuleFromNodeId(id_input);
    if(nodeOneModule === nodeTwoModule) {
      ...
      // Check connection exist
      var exists = this.drawflow.drawflow[nodeOneModule].data[id_output].outputs[output_class].connections.findIndex(matches);
      if(exists > -1) {

        if(this.module === nodeOneModule) {
          // In same module with view.
          var selector = connection_id !== null
            ? '.connection[data-edge-id="'+connection_id+'"]'
            : '.connection.node_in_node-'+id_input+'.node_out_node-'+id_output+'.'+output_class+'.'+input_class;
          this.container.querySelector(selector).remove();
        }

        var index_out = this.drawflow.drawflow[nodeOneModule].data[id_output].outputs[output_class].connections.findIndex(matches);
        this.drawflow.drawflow[nodeOneModule].data[id_output].outputs[output_class].connections.splice(index_out,1);

        var index_in = this.drawflow.drawflow[nodeOneModule].data[id_input].inputs[input_class].connections.findIndex(matchesIn);
        this.drawflow.drawflow[nodeOneModule].data[id_input].inputs[input_class].connections.splice(index_in,1);

        this.dispatch('connectionRemoved', { output_id: id_output, input_id: id_input, output_class:  output_class, input_class: input_class, id: connection_id});
        return true;
